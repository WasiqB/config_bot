  ...
  class New
    ...
    def set_question index, prefix = ""
      ...
      type = prompt.select "#{question_id}.2. What is the type of question u want to ask?", \
        Common::TYPES, required: true, default: 1
      ...
      question.required = prompt.yes? "#{question_id}.5. Is your question mandatory?", \
        default: false, convert: :bool
      case question.type
      when "mask"
        question.default = prompt.ask "#{question_id}.6. What default value you expect for your question?", \
          convert: question.convert
        question.mask = prompt.ask "#{question_id}.7. What mask value you want for your question?", \
          required: true
      when "yes?"
        question.default = prompt.ask "#{question_id}.6. What default input you expect for your question?", \
          convert: question.convert
        question.positive = prompt.ask "#{question_id}.7.1. What is the positive value you want?", \
          required: true
        question.negative = prompt.ask "#{question_id}.7.2. What is the negative value you want?", \
          required: true
      when "select", "multi_select", "enum_select"
        choice_count = prompt.ask "#{question_id}.6.1. How many choices are there for your question?", \
          required: true, convert: :int
        (1..choice_count).each do |i|
          item = prompt.ask "#{question_id}.6.1.#{i}.1. Add choice:", required: true
          id = prompt.ask "#{question_id}.6.1.#{i}.2. Choice id:", \
            required: true, convert: question.convert
          choice = Choice.new item, id
          question.add_choice choice
        end
        question.help_msg = prompt.ask "#{question_id}.6.2. What help message u want?"
        question.default = prompt.ask "#{question_id}.7. What default choice id you expect for your question?", \
          convert: :int
      when "slider"
        question.default = prompt.ask "#{question_id}.6. What default value you expect for your question?", \
          convert: question.convert
        question.min = prompt.ask "#{question_id}.7.1. What is the min value for slider?", \
          required: true, convert: :int
        question.max = prompt.ask "#{question_id}.7.2. What is the max value for slider?", \
          required: true, convert: :int
        question.step = prompt.ask "#{question_id}.7.3. What is the step value for slider?", \
          required: true, convert: :int
      end
      case question.type
      when "yes?", "select", "multi_select", "enum_select"
        if prompt.yes? "#{question_id}.8. Do your question expects any results?", \
            required: true, convert: :bool
          result_count = prompt.ask "#{question_id}.8.1. How many results are there?", \
            required: true, convert: :int
          (1..result_count).each do |i|
            value = prompt.ask "#{question_id}.8.1.#{i}.1. What value this result can have?", \
              required: true, convert: question.convert
            result = Result.new value
            question_count = prompt.ask "#{question_id}.8.1.#{i}.2. How many questions your result have?", \
              required: true, convert: :int
            (1..question_count).each do |id|
              result.questions.push set_question id, "#{question_id}.8.1.#{id}.2."
            end
